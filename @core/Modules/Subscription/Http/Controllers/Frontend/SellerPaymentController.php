...
class SellerPaymentController extends Controller
{
    ...
    public function kineticpay_ipn_for_subs()
    {
    ...
    public function send_subscription_mail($order_id)
    {
        ...
        try {
            ...
            $message = str_replace(["@type","@price","@connect", "@service", "@job"],[$subscription_details->type,float_amount_with_currency_symbol($subscription_details->price),$subscription_details->connect, $subscription_details->service, $subscription_details->job],$message);
            ...
            $message = str_replace(["@type","@price","@connect","@service", "@job", "@seller_name","@seller_email"],[$subscription_details->type,float_amount_with_currency_symbol($subscription_details->price),$subscription_details->connect,$subscription_details->service,$subscription_details->job,$user_name,$user_email],$message);
    ...
    private function update_database($subscription_id, $transaction_id, $history_id)
    {
        ...
        $subscription_history = SubscriptionHistory::find($history_id);

        if($subscription_details){
            // check seller old subscription expire date calculate
            $seller_current_subscription_check = SellerSubscription::where('subscription_id', $subscription_details->subscription_id)
            ...
            if ($seller_current_subscription_check){
                // days add
                $expireDate = Carbon::parse($subscription_details->expire_date);
                $remaining_expire_days = $expireDate->diffInDays(now());

                if ($subscription_history->type == 'monthly') {
                    $expire_date = Carbon::now()->addDays(30 + $remaining_expire_days);
                } elseif ($subscription_history->type == 'yearly') {
                    $expire_date = Carbon::now()->addDays(365 + $remaining_expire_days);
                } elseif ($subscription_history->type == 'lifetime') {
            ...
            SellerSubscription::where('id', $subscription_id)->update([
                ...
                'connect' => ($subscription_history->connect),
                'service' => ($subscription_history->service),
                'job' => ($subscription_history->job),
