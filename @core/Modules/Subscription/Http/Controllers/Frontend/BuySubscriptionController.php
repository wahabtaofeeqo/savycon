...
class BuySubscriptionController extends Controller
{
    ...
    public function buy_subscription(Request $request)
    {
        ...
        if (!isset($subscription)) {
            toastr_error(__('Subscription not found.'));
        ...
        if(isset($subscription->id)){
            ...
            // if subscription payment gateway not select
            if($subscription->price > 0){
            ...
            // Find seller subscription if not expired
            $seller_subscription = SellerSubscription::where('seller_id', $seller_id)
                ->where('expire_date', '>=', now())
                ->where('payment_status', 'complete')
                ->first();


            $seller_subscription_if_expire = SellerSubscription::where('seller_id', $seller_id)
                ->where('expire_date', '<=', now())
                ->where('payment_status', 'complete')
                ->first();

            if ($seller_subscription_if_expire) {
                $seller_subscription_if_expire->update([
                    'connect' => 0,
                    'service' => 0,
                    'job' => 0,
                ]);
            ...
            // Seller Check if subscription exists and expire date
            $seller_subscription_connect = 0;
            $seller_subscription_service = 0;
            $seller_subscription_job = 0;
            ...
            if ($seller_subscription) {
                if($seller_subscription->type=='monthly'){
                    ...
                    $seller_subscription_connect = $seller_subscription->connect;
                    $seller_subscription_service = $seller_subscription->service;
                    $seller_subscription_job = $seller_subscription->job;
                ...
                if($subscription->type=='monthly'){
                    ...
                    $service = $subscription->service;
                    $job = $subscription->job;
                }elseif($subscription->type=='lifetime'){
                ...
                if(!empty($get_coupon_code)){
                    if($get_coupon_code->expire_date > $current_date){
                        if($get_coupon_code->discount_type == 'percentage'){
                            $discount = ($subscription->price * $get_coupon_code->discount)/100;
            ...
            if($request->seller_payment_later != 'later'){
                if(Auth::guard('web')->check()){
                    ...
                    $seller_id_from_seller_subscription_table = SellerSubscription::select('id','seller_id','connect', 'service', 'job', 'expire_date')
                        ->where('seller_id',$user_id)
                        ->first();

                    // seller old subscription find
                    $seller_pending_history = SubscriptionHistory::find($request->history_id);
                    if (!empty($seller_pending_history) && $seller_pending_history->payment_status != 'complete') {
                        $create_history = $seller_pending_history;
                    }else{
                        $create_history = SubscriptionHistory::create([
                            'subscription_id' => $subscription->id,
                            'seller_id' => Auth::guard('web')->user()->id,
                            'type' => $subscription->type,
                            'service' =>  $service + $seller_subscription_service,
                            'job' =>  $job + $seller_subscription_job,
                            'connect' => $connect + $seller_subscription_connect,
                ...
                if ($request->selected_payment_gateway === 'wallet') {
                    ...
                    if($subscription_details){
                        SellerSubscription::where('id', $last_subscription_id)->update([
                            ...
                            'service' => $subscription_details->initial_service + $subscription_details->service,
                            'job' => $subscription_details->initial_job + $subscription_details->job,
                    ...
                    try {
                        $connect = $subscription->type =='lifetime' ? __("No Limit") : $subscription->connect;
                        ...
                        $message = str_replace(["@type","@price","@connect","@service", "@job"],[$subscription->type,float_amount_with_currency_symbol($price),$connect, $service, $job],$message);
                        ...
                        $message = str_replace(["@type","@price","@connect","@service", "@job","@seller_name","@seller_email"],[$subscription->type,float_amount_with_currency_symbol($price),$connect,$service,$job,$user_name,$user_email],$message);
            ...
            if($request->selected_payment_gateway === 'manual_payment') {
                ...
                try {
                    ...
                    $message = get_static_option('buy_subscription_admin_message') ?? '';
                    $message = str_replace(["@type","@price","@connect","@service", "@job","@seller_name","@email"],[$subscription->type,float_amount_with_currency_symbol($price),$connect,$service,$job,$user_name,$user_email],$message);
                    Mail::to(get_static_option('site_global_email'))->send(new BasicMail([
                        'subject' =>get_static_option('buy_subscription_email_subject') ?? __('New Subscription'),
                        'message' => $message
                    ]));
                ...
                if ($request->selected_payment_gateway === 'paypal') {
                    ...
                    $redirect_url = $paypal->charge_customer([
                        ...
                        'title' => $subscription->type, // payment title
