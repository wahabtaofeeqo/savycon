...
class WalletController extends Controller
{
    ...
    {
        update_static_option('wallet_deposit_max_value',$request->wallet_deposit_max_value);
        update_static_option('wallet_deposit_min_value',$request->wallet_deposit_min_value);
    ...
    public function depositCreateByAdmin(Request $request)
    {
        ...
        $deposit_max_value = get_static_option('wallet_deposit_max_value') ?? 50000;
        $deposit_min_value = get_static_option('wallet_deposit_min_value') ?? 10;

        if ($request->amount < $deposit_min_value || $request->amount > $deposit_max_value) {
            return back()->with('msg', __('Invalid amount. Please enter an amount between') . ' ' . $deposit_min_value . ' and ' . $deposit_max_value . '.')->with('type', 'danger');
        }
