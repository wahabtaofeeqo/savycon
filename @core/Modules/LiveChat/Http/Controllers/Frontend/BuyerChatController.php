...
class BuyerChatController extends Controller
{
    ...
    {
        $livechat_seller_message_ids = LiveChatMessage::select('id', 'seller_id')->pluck('seller_id');
        $users = User::where('id', '!=', Auth::guard('web')->user()->id)
            ->where('user_type', 0)
            ->whereIn('id', $livechat_seller_message_ids)
            ->get();
    ...
    public function postSendMessage(Request $request)
    {
        ...
        if($request->hasFile("image")) {
            ...
            if ($filename) {
                ...
                return response()->json(['error' => __('Failed to upload image.')]);
            }
        }
